...
-}
strSum :: [Char] -> Int
strSum = sum . map (read::String->Int) . words
...
lowerAndGreater n list = show n ++ " is greater than " ++ show g ++ " elements and lower than " ++ show l ++ " elements"
 where g = length (filter (<n) list)
       l = length (filter (>n) list)
